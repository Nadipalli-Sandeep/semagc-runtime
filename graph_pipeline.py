...
from semagc_runtime import SemaGCState
...
# ── Node 1: Task Planner ──────────────────────────────────────────────────────
def planner_node(state: SemaGCState) -> dict:
    time.sleep(4)
...
def code_gen_node(state: SemaGCState) -> dict:
    time.sleep(4)
...
# ── Node 3: Debugger ──────────────────────────────────────────────────────────
def debugger_node(state: SemaGCState) -> dict:
    time.sleep(4)
    history = "\n".join([m.content for m in state["memory_vault"]])[-2000:]
    response = llm.invoke([
...
def reviewer_node(state: SemaGCState) -> dict:
    time.sleep(4)
    history = "\n".join([m.content for m in state["memory_vault"]])[-2000:]
    response = llm.invoke([
...
# ── Node 5: Test Writer ───────────────────────────────────────────────────────
def test_writer_node(state: SemaGCState) -> dict:
    time.sleep(4)
    history = "\n".join([m.content for m in state["memory_vault"]])[-2000:]
    response = llm.invoke([
...
def summarizer_node(state: SemaGCState) -> dict:
    time.sleep(4)
    history = "\n".join([m.content for m in state["memory_vault"]])[-2000:]
    response = llm.invoke([
...
# ── Build the graph ───────────────────────────────────────────────────────────
def build_semagc_graph():
    builder = StateGraph(SemaGCState)
...
if __name__ == "__main__":
    ...
    config = {"configurable": {"thread_id": "semagc-run-1"}}
